...
const audio = document.getElementById("sunsetAudio");
...
function animate(c) {
    ...
    const twoColGradient = (color1, color2) => {
        const gradient = [];
    ...
    let sunsetcolorScheme = twoColGradient(
		{r: 161, g: 218, b: 250},
		{r: 254, g: 216, b: 213}
    );
    ...
    const updateImage = () => {
        for (let row = 0; row < imgSize; row++) {
            for (let col = 0; col < imgSize; col++) {
				...
				const pixelColor = sunsetcolorScheme[h];
